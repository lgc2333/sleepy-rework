...
export default function Switch({
  ...
  const iconOnStyle = useAnimatedStyle(() => ({
    opacity: disabled
      ? 0.38
      : interpolate(position.value, [0, 10], [0, 1], Extrapolation.CLAMP),
    overflow: 'hidden',
    transform: [
      {
  ...
  const iconOffStyle = useAnimatedStyle(() => ({
    ...
    opacity: disabled
      ? 0.38
      : interpolate(position.value, [-10, 0], [1, 0], Extrapolation.CLAMP),
    overflow: 'hidden',
    transform: [
      {
  ...
  return (
    <View style={[{ borderRadius: 20, backgroundColor: theme.colors.surface }, style]}>
          ...
          {switchOnIcon ? (
            <Animated.View key={10} style={iconOnStyle}>
              <Icon
                ...
                size={16}
                color={
                  disabled ? theme.colors.onSurface : theme.colors.onPrimaryContainer
                }
              />
            </Animated.View>
          ) : null}
